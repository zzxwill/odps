...
-   支持OUTER LATERAL VIEW
    ...
    MaxCompute Sprint30扩展了Lateral View功能，支持Outer Lateral View，当table function不输出任何一行时，对应的输入行在lateral view结果中依然保留，且所有table function输出列为 null。关于Lateral View介绍请参考文档[DML语句](../../../../cn.zh-CN/用户指南/SQL/INSERT操作/Lateral View.md#)的“Lateral View”部分。
...
-   支持SCALAR SUBQUERY
    ...
    子查询支持SCALAR SUBQUERY，即当SUBQUERY的输出结果为单行单列的时候，可以当做标量来使用。具体使用介绍请参考文档[DML语句](../../../../cn.zh-CN/用户指南/SQL/SELECT操作/SCALAR SUBQUERY.md#)的“SCALAR SUBQUERY”部分。
