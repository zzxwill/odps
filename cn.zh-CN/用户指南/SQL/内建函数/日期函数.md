...
本文向您介绍MaxCompute SQL提供针对Datetime类型的参数的日期函数命令操作。
...
-   datepart：String类型常量。此字段的取值遵循String与Datetime类型转换的约定，即yyyy表示年，mm表示月。
    ...
    关于类型转换的规则，请参见[String类型与Datetime类型之间的转换](cn.zh-CN/用户指南/SQL/SQL概述.md)。此外也支持扩展的日期格式：年-year，月-month或mon，日-day，小时-hour。非常量、不支持的格式或其它类型会抛异常。
