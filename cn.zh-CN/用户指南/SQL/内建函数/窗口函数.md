...
```
SELECT ROW_NUMBER() OVER w, RANK() OVER w FROM t  WINDOW w as (partition by c1 order by c2);
```

**说明：** WINDOW语句定义出现在where/groupby/having语句之后，在order by/cluster by/distribute by/sort by/limit语句之前。
